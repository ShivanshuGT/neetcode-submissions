class Solution {
    ...
    private void dfsModified(List<List<Integer>> graph, int[] visited, int node, List<Integer> ans){
        visited[node] = 1;
        ans.add(node);
        ...
        for(int neighbour : neighbours){
            ...
            visited[neighbour] = 0;
            dfsModified(graph, visited, neighbour, ans);
        }
    ...
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        if(hasCycle(graph, numCourses)){
            ...
            for(int i = 0; i< numCourses; i++){
                if(visited[i] == 0){
                    dfsModified(graph, visited, i, ans);
                }
