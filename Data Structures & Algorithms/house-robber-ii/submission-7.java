class Solution {
    ...
    private int spaceOptimizedHelper(int[] nums){
        int n = nums.length;


        int prev = Math.max(nums[0], nums[1]);
        int prePrev = nums[0];
        int ans = Math.max(prev, prePrev);

        for(int i = 2; i < n; i++){
            ans = Math.max(prePrev + nums[i], prev);
            prePrev = prev;
            prev = ans;
        }
        return ans;
    ...
    private int spaceOptimized(int[] nums){
        ...
        int first = spaceOptimizedHelper(Arrays.copyOfRange(nums, 0, n-1));
        int second = spaceOptimizedHelper(Arrays.copyOfRange(nums, 1, n));
        return Math.max(first, second);
    }
